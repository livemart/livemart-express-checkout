...
const OptionSelector = ({ product }) => {
  ...
  useEffect(() => {
    livemartStorage.set(`${product.id}_quantity`, `${quantity}`)

    const opts = []
    for (const option of product.attributes) {
      const opt = {
        title: option.name,
        id: option.id,
    ...
    livemartStorage.saveAttribute(JSON.stringify(select))
  }, [product])
  ...
  const handleSelect = e => {
    ...
    livemartStorage.saveAttribute(JSON.stringify(tmp))
  }
